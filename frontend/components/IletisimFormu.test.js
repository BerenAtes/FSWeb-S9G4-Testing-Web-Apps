...
test("kullanıcı doğru ad ve soyad girdiğinde ama email girmediğinde BİR hata mesajı render ediliyor.", async () => {
  ...
  userEvent.type(adInput, "Veysel");
  userEvent.type(soyadInput, "Yıldırım");
...
test('soyad girilmeden gönderilirse "soyad gereklidir." mesajı render ediliyor', async () => {
  render(<IletisimFormu />);
  const adInput = screen.getByLabelText("Ad*");
  userEvent.type(adInput, "Beren");
  const email = screen.getByLabelText(/Email\*/i);
  userEvent.type(email, "berenates@gmail.com");
  const button = screen.getByRole("button", { name: /Gönder/i });
  userEvent.click(button);
...
test("form gönderildiğinde girilen tüm değerler render ediliyor.", async () => {
  render(<IletisimFormu />);
  userEvent.type(screen.getByPlaceholderText("İlhan"), "Beren");
  userEvent.type(screen.getByPlaceholderText("Mansız"), "Ateş");
  userEvent.type(
